...
每次 Provider 响应完成后，CLI 会显示该轮由服务商 `usage` 返回的输入、缓存和输出 token；同一任务的累计用量由各轮已返回的指标相加得到。TriCoder 只归一化并展示/审计这些服务商返回的用量数据，不会在本地保存或管理 KV Cache。

缓存相关字段是观测值，不是本地缓存状态：服务商未返回某个字段时，界面会显示 `-`，不会将未知值当作 `0`。缓存命中率仅在服务商同时返回可计算的输入和缓存 token 时显示。
