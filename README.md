...
TriCoder 可把已闭合的完整任务块整理为结构化的目标、约束、决策和待办，同时保留近期完整对话。运行时上下文压缩只处理较早历史；待保存候选则独立覆盖旧记忆边界之后的全部已完成任务，包括最新完成的任务，不会为了保存而删除仍在内存中的近期消息。工具调用与结果必须成组进入候选，摘要失败、取消或候选校验失败都不会先删除历史。模型生成的记忆不包含也不能改变权限、审批、验证证据、修改日志或 `unknown_effects`；这些可信执行状态始终独立维护。

摘要 Provider 只生成 `goal`、`constraints`、`decisions` 与 `open_items` 四类低信任语义字段；schema 版本、revision、generation 和覆盖位置由本地程序填写。schema v2 为条目增加状态：目标/约束/有效决策为 `active`，待办为 `pending/done/cancelled`，被替代目标或决策为 `superseded`。终结或被替代条目进入最多 40 条的归档，归档会出现在精确保存预览并随确认保存，但默认不注入模型请求。旧 schema v1 可正常加载，待办映射为 `pending`，下一次确认保存时才写成 v2；未知版本仍会拒绝。

同 ID 的约束修改、待办终结、决策替代和新任务目标只会先形成待保存候选，必须经过 `/memory save` 的精确预览确认才会落库；运行时压缩仍采用保守合并，不让模型直接改写既有条目。相同的决策替代或终结状态被摘要器重复返回时按幂等重放处理，不重复归档；冲突的 ID、替代关系或语义仍会严格拒绝。用户约束不会被自动遗忘；每类最多 20 条活跃条目，已终结内容可归档，活跃内容仍超限时会拒绝候选并要求用户明确选择。这里的 `done` 只表示语义待办状态，不是测试通过或工具成功的可信证据。

解析器接受纯 JSON，以及包住整个响应且不带额外说明的单个 JSON 代码围栏，仍会拒绝未知字段、伪造来源、工具调用和截断响应。失败时 CLI 只显示固定诊断类别（例如 JSON 格式无效、响应截断或 Provider 请求失败），不会展示或审计模型原文；原始历史与现有记忆保持不变。
...
如需重启恢复，再把 `persistence` 改为 `"reviewed_summary"`。此模式在每个成功闭合任务后生成或扩展待审候选，但不会自动写入 SQLite；使用 `/memory` 查看当前候选，`/memory edit <条目ID>` 输入文本、scope 和状态后在本地预览并确认修改，`/memory save` 查看数据库位置、完整覆盖范围和精确 JSON 后再次确认保存。普通保存只接受覆盖最新可信已完成任务水位的候选；候选落后时会拒绝，并提示执行 `/memory refresh`。刷新只调用无工具记忆摘要器，不重跑业务工具也不直接写库；成功后仍需 `/memory save`，失败、取消或两批中任一批失败都保留原候选与近期历史。刷新会产生额外的记忆模型请求；候选已完整时则直接复用，不增加调用。若旧候选落后于已经压缩且无法恢复来源的运行时记忆，刷新会明确拒绝，不能伪造完整覆盖。

持久化只接受经过校验、未命中敏感内容检查的结构化字段与有界归档，不保存源码正文、原始工具输出、真实凭据或可信执行状态。记忆整理会增加额外模型调用和 token 用量，CLI 将其与业务调用分开统计。`/memory archive` 只列出归档 ID、类别、状态和条目/字符容量，不显示正文；`/memory archive delete <条目ID>` 会先展示绑定当前 Session、generation、revision 和原始快照的删除预览，确认后只修改当前内存候选，不自动保存。要让删除跨重启生效，必须再次执行 `/memory save`。确认保存不会立即用候选替换本进程的运行时摘要或近期消息；重启后才从已保存候选恢复结构化记忆，因此不会在当前进程重复注入“候选摘要＋对应近期原文”。
...
同一批调用首次失败后，剩余动作全部标记为 `skipped`：不审批、不执行、不计入实际执行次数。取得当前工具结果后，在本地协议消息构造成功的边界内，每个合法 call ID 恰有一个结果，配对不依赖完成事件、结果观察者或审计通知成功；通知异常停止任务并保留首个异常。若协议构造本身失败，则直接停止、不重试、不向模型发送不完整回合，也不声称已完成全部配对。只有根错误允许 `replan` 且文件影响可确认时才把完整回合交回模型，由下一轮重新决策；不会自动重试旧批次。审批拒绝、取消、清理失败、`stop_task` 或 UNKNOWN 文件影响会直接停止任务。成功 `finish` 后的同批动作也只配对、不执行。当前不推断动作依赖，也不并行执行工具；`legacy_json` 仍每轮只接受单动作。
...
UNKNOWN 会以 SQLite 布尔元数据保存；若本地变更账本已经确认了相关规范相对路径，
这些路径也会沿用现有 `modified_files` 字段持久化，供 `/status` 和 `/diff` 提示检查范围。
数据库不保存冲突证据、源码正文、文件快照或外部工具自报路径；无法由本地账本确认的
路径不会因模型或 MCP 返回同名字段而取得信任。旧库会在事务内幂等补齐 UNKNOWN 列，
不删除原有记录。不要让旧版和新版程序同时操作同一数据库；回退代码可能忽略
UNKNOWN，不能视为安全语义上的兼容。内置文件工具记录可观察的副作用；认可检查命令
只能证明下述受覆盖文件范围，不保证普通命令或外部 MCP 的全部影响已被追踪。

## 文件状态检查证据

命令退出码 `0` 只表示该命令成功结束。只有内置 `run_command` 执行受策略认可的
`python -m unittest|pytest|compileall|ruff|mypy`，且命令前后扫描完整、文件状态一致、
进程清理完成且未取消时，才形成受覆盖文件状态的本地证据；这不是业务需求验收，
也不保证测试充分或代码正确。普通 Python 脚本、一般外部命令和非只读 MCP/扩展
没有这种证明，运行后按文件影响 UNKNOWN 处理并停止任务；需人工检查后以 `/clear`
明确确认才能继续。内置只读工具仍可按原有规则使用。

扫描绑定规范工作区、本地 Session 能力和版本化范围规则，覆盖普通文件的相对路径、
类型、内容、大小、权限及可用身份。固定排除 `.git`、`.venv`、`__pycache__`、
`.pytest_cache` **目录**以及当前本地审计器持有的精确日志文件；不采用 `.gitignore`
来隐藏普通 `runtime/`、源码、测试或配置。敏感路径不读正文，但会使覆盖不完整；
链接、junction/reparse、权限错误或扫描竞争也不能静默略过后通过。

默认预算为 10,000 个条目（包含目录）、总计 100 MiB、单文件 8 MiB、单次扫描
3 秒。超限保守失败。时间预算为协作式检查，阻塞的文件系统调用可能超过预算；
Windows 安全句柄成本也会使较大工作区提前达到时间上限。证据与内部清单只驻内存，
不会写入 SQLite、审计、普通日志或模型消息，digest 也不发送给模型。

`finish` 会重新扫描；写入、外部新增/删除/修改、范围变化、取消或清理失败都会使
旧证据失效。相同文件版本的失败检查不能被随后无关成功覆盖，新版本才可重新检查。
只有完整、同范围的两次快照明确不同，或本地已确认写入，才算证明新版本；一次
不可读、不完整扫描或范围变化不能解除已有失败。工具结果后的观察者/审计异常仍
原样停止任务，但已在本地确认的失败和 UNKNOWN 不会随异常丢失或恢复成旧通过。
异常对账按任务内提交修订区分已消费的旧写入和新的未交付写入；后续只读工具中断
不能把旧写入重放为“新版本”。非只读扩展通过参数、权限、审批和预取消检查后，
一旦交给 handler 即保守登记 UNKNOWN；即使外部调用抛中断、没有返回结果也保留。
异步 handler 在交付后才检查取消的竞争窗口同样保守处理，未批准或预取消不会登记。
Runtime 在任何 Agent 启动前建立任务基线，正常返回和异常收尾都会合并本地已发布
的验证事实；自定义 Agent 返回旧上下文或自报成功，不能覆盖真实失败或伪造通过。
Runtime 接受通过证据前还会用当前本地 scope 再扫描；不能用证据自身保存的快照
证明它仍新鲜。扫描不完整或普通扫描异常均保守拒绝通过，不解除已有失败。
最后成功门槛独立核对任务取消令牌，纯读任务取消也不能成功；取消撤销旧 pass，
保留原失败原因。取消同时伴随清理失败或 UNKNOWN 时，结果仍以“任务已取消”为
主原因，并追加资源或文件需人工核对的提示；不会被通用 UNKNOWN 文案覆盖。
账本封存及结果准备完成后、持久化之前，Runtime 会在同一状态锁
下读取取消状态并关闭取消接收；此前接受的取消必定使提交结果失败，之后的取消
请求返回未接受，但任务仍负责持久化与退出清理，期间不会允许新任务复用资源。
最终扫描若被 SystemExit 等中断，先撤销通过能力、保留已有失败，再原样传播异常；
这不改变普通工具通知异常后保留本地已发布事实的规则。
已有主异常时，补偿封存或补偿持久化再次被中断不会覆盖首异常；保存未完成时保留
待保存状态。这个保护只用于异常后的补偿收尾，正常执行中的中断仍原样抛出。
Runtime 最终扫描由同步任务线程在现有 cleanup scope 内完成，
返回前不释放任务所有权；增加至多一次复核扫描，沿用上述协作式预算，非硬截止。
输出处理中发生结构化取消时，也先保留已发布失败，再应用取消终态。新提交即使
把文件写回任务开始时的内容，也立即撤销旧证据；净零本身不生成撤销变更组。
纯读取任务仍可无测试完成，但已有陈旧证据或失败记录不能通过空修改列表绕过。
同一 Session 连续轮次可重新核对内存证据；切 Session、重启、`/clear` 和撤销均不
恢复历史“通过”。`/status` 显示的是上次文件状态检查，后续外部修改仍须再核验。

快照返回到实际使用文件之间仍存在 TOCTOU 窗口；批准的恶意程序、脱离管理的后代
和 OS 沙箱不属于该证明范围。目前只在 Windows 离线 fixture 验证，Linux/POSIX、
真实 Provider 和真实外部 MCP 未验证。

## 前五项可靠性集成验收

I01–I07 已用七条独立公共流程串联验证：失败补丁的残留与修复、UNKNOWN 的本地路径
可见性、审批等待取消、受管子进程取消与清理失败、验证后外部修改、Session 隔离／
重启／撤销，以及 native／legacy 调用 ID 配对。验收使用临时工作区、临时 SQLite 和
审计文件，不读取真实项目源码作为扫描目标。最终整体复审还覆盖了原生取消后的晚到
写入、缺失路径错误分流、MCP 冷导入顺序和存活进程取消回收；`MCPToolHandler` 的旧
包级导入通过惰性导出保持兼容。完整串行测试在 Windows 上运行 1,077 项并通过，6 项
按平台能力跳过。详细 RED／GREEN 证据与仍未验证的边界见计划、本地
`runtime/reliability-top5/integration-report.md` 和 `final-remediation-review.md`。
